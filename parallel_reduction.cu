...
int main()
{
	...
	int blocks_needed = threads_needed/(threadsPerBlock) + 1;
	int needed_shared_memory = threads_needed * sizeof(float);

	auto tracker = MemoryTracker<float>();
	auto host_input = tracker.allocate_host_memory(size, "host_input");
	auto host_output = tracker.allocate_host_memory(blocks_needed, "host_output");
	...
	auto device_output = tracker.allocate_device_memory(blocks_needed, "device_output");
	tracker.copy_host_array_to_device_array(host_input, device_input, size);

	parallel_array_sum_v0<float><<<blocks_needed, threadsPerBlock, needed_shared_memory>>>(device_input, device_output, size);
