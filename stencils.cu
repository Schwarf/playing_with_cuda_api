...
int main()
{
	...
	auto host_output = tracker.allocate_host_memory(size, "host_output");
	get_random_float<float>(0.f, 100.f, host_input, size);
	...
	auto device_output = tracker.allocate_device_memory(size, "device_output");
	tracker.copy_host_array_to_device_array(host_input, device_input, size);

	int threadsPerBlock = 256;
	int blocksPerGrid = (size + threadsPerBlock - 1) / threadsPerBlock;
	float needed_shared_memory = (threadsPerBlock + 2) *sizeof(float);
	...
	tracker.copy_device_array_to_host_array(host_output, device_output, size);

	for (int i = 0; i < size - 2 * k; ++i) {
		std::cout
			<< host_expected_output[i]/host_output[i] << "  " << host_output[i] << std::endl;
		//std::cout << (A[i] + A[i+1] + A[i+2] )/3 / B[i] << std::endl;
	}
